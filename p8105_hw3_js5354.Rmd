...
pinklady_hof = pinklady$mean_hof
...
instacart %>% 
  ...
  summarize(mean_hof = mean(order_hour_of_day)) %>% 
  mutate(mean_hof = paste(floor(mean_hof), round((mean_hof-floor(mean_hof))*60), sep=":")) 
coffee_icecream_hof = coffee_icecream$mean_hof

# create a 2*7 tibble
as_tibble(rbind(pinklady_hof, coffee_icecream$mean_hof)) %>% 
  rename (Sun = V1, Mon = V2, Tue = V3, Wed = V4, Thu = V5, Fri = V6, Sat = V7) 
  
```


  The table above lists the average hour of day when pink lady apples and coffee ice cream are ordered, based on the `instacart` data. The first row is associated with pink lady apples and the second row is related to coffee ice cream. In general, the two items are mainly ordered in noon and early afternoon, in a window between 11:00 to 14:30. Differences in the mean hour of day among different weekdays are subtle. 
